...
#     project = ProjectSerializer(read_only=True,many=False)
    project = ModelRelatedField(model=Project,serializer=ProjectSerializer,required=True)
    lab = ModelRelatedField(model=Lab,serializer=LabSerializer,required=False,allow_null=True)
...
#     participants = ManyUserField(queryset=User.objects.all())
    class Meta:
        ...
        fields = ('id','type','name','project','lab','created','manager','participants','description','data_location')
